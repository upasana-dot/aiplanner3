...
const ChatWidget: React.FC<ChatWidgetProps> = ({ destination }) => {
    ...
    return (
        <>
            ...
            {isOpen && (
                <div className="fixed bottom-24 right-6 w-full max-w-sm h-[70vh] max-h-[500px] bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl flex flex-col animate-fade-in z-40 ring-1 ring-black/5">
                    ...
                    {isSubmitted ? (
                        <div className="flex-grow flex flex-col items-center justify-center p-4 bg-gray-50/80 text-center">
                            <h4 className="text-2xl font-bold text-blue-600">Thank You!</h4>
                            <p className="text-gray-600 mt-2">Have a great day!</p>
                        </div>
                    ) : (
                        <>
                            <div className="flex-grow p-4 overflow-y-auto bg-gray-50/80">
                            ...
                            <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-200 flex items-center bg-white/80 rounded-b-3xl">
                                <input
                                    type="text"
                                    value={input}
                                ...
                                <button type="button" onClick={handleDone} className="ml-2 text-sm font-semibold text-gray-600 hover:text-black">End Chat</button>
                            </form>
                        </>
                    )}
                </div>
