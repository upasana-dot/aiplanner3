...
const ActivityRow: React.FC<{ activity: Activity }> = ({ activity }) => (
    <div className="flex flex-col md:flex-row gap-4 bg-white/80 p-4 rounded-xl border border-gray-200/80 shadow-sm transition-shadow hover:shadow-md overflow-hidden">
        {activity.imageUrl ? (
            <img src={activity.imageUrl} alt={activity.description} className="w-full md:w-56 h-40 object-cover rounded-lg flex-shrink-0" />
        ) : (
            <div className="w-full md:w-56 h-40 bg-gray-200 rounded-lg flex-shrink-0 flex items-center justify-center">
                <MapPinIcon className="w-12 h-12 text-gray-400" />
            </div>
        )}
        <div className="flex-grow space-y-2 py-1">
            <p className="text-gray-800 font-semibold text-lg">{activity.description}</p>
            ...
            <div className="border-t border-gray-200 pt-3 space-y-2 text-sm mt-2">
...
const DayPlanCard: React.FC<{ dayPlan: DailyPlan }> = ({ dayPlan }) => (
    <div className="bg-white/90 backdrop-blur-xl p-6 rounded-3xl shadow-xl ring-1 ring-black/5 transition-shadow hover:shadow-2xl">
...
const TripEssentials: React.FC<{ itinerary: ItineraryData }> = ({ itinerary }) => {
    return (
        <div className="bg-white/90 backdrop-blur-xl p-6 rounded-3xl shadow-xl ring-1 ring-black/5 mt-8">
    ...
    return (
        <div className="animate-fade-in space-y-8">
            <div id="itinerary-content">
                <header className="bg-white/90 backdrop-blur-xl text-center rounded-3xl shadow-xl shadow-gray-900/10 ring-1 ring-black/5 relative overflow-hidden">
